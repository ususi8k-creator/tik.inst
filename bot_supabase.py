...
# ===== Supabase Virtual Storage Layer =====
import os, json, io
...
def _get_record(name):
    res = supabase.table(TABLE_NAME).select("*").eq("name", name).execute()
    if res.data:
        return res.data[0]
    return None

def _set_record(name, text=None, data=None):
    payload = {"name": name, "text_data": text, "json_data": data}
    supabase.table(TABLE_NAME).upsert(payload).execute()

class SupabaseFile(io.StringIO):
    def __init__(self, name, mode):
        self.name = name
        self.mode = mode
        rec = _get_record(name)
        content = ""
        if rec:
            if rec.get("text_data"):
                content = rec["text_data"]
            elif rec.get("json_data"):
                content = json.dumps(rec["json_data"])
    ...
    def close(self):
        if "w" in self.mode or "a" in self.mode:
            ...
            try:
                js = json.loads(value)
                _set_record(self.name, data=js)
            except:
                _set_record(self.name, text=value)
        super().close()
